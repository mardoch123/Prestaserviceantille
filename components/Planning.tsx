...
const Planning: React.FC = () => {
  const { missions, providers, clients, addMission, assignProvider, deleteMissions } = useData(); 
  ...
  // Form State - Updated to use clientId and recurrence
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      // Determine End Time (same for all)
      const [h, m] = missionForm.startTime.split(':').map(Number);
      const duration = parseInt(missionForm.duration);
      ...
      if (!client) { alert("Client invalide"); return; }

      // Recurrence Logic
      const startDateObj = new Date(missionForm.date);
      if(isNaN(startDateObj.getTime())) { alert("Date invalide"); return; }
      ...
      for (let i = 0; i < count; i++) {
          ...
          // STRICT COLLISION CHECK LOGIC
          const collision = missions.find(m => 
              (m.status === 'planned' || m.status === 'in_progress') && 
              m.date === dateStr && 
              ((missionForm.startTime < m.endTime && endTime > m.startTime))
          );

          if (collision) {
              if (i === 0) {
                  alert(`❌ Créneau indisponible le ${dateStr} !\n\nLe créneau est déjà bloqué par (${collision.clientName}).`);
                  return;
              } else {
                  const confirmContinue = window.confirm(`❌ Conflit de planning le ${dateStr} (occurrence ${i+1}/${count}).\n\nLe créneau est déjà bloqué par (${collision.clientName}).\n\nVoulez-vous arrêter la planification de la série ici ?`);
      ...
      showToast(createdCount > 1 ? `${createdCount} missions récurrentes planifiées !` : 'Mission planifiée et créneaux bloqués !');
      // Reset form
      setMissionForm({ clientId: '', service: '', providerId: '', date: '', startTime: '09:00', duration: '2', recurrence: 'none', occurrences: 1 });
  };
  ...
  const handleConfirmAssignment = () => {
      if (selectedMissionId && assignProviderId) {
          ...
          if (mission && provider) {
              assignProvider(mission.id, provider.id, `${provider.firstName} ${provider.lastName}`);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[80vh]">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">Durée (h)</label>
                            <input 
                                required
                                type="number" 
                                name="duration"
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        ...
                        >
                            <CheckCircle className="w-4 h-4" /> Planifier
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm animate-in fade-in zoom-in duration-200">
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-slate-500 mb-6">
                        Êtes-vous sûr de vouloir supprimer {selectedMissionIds.size} mission(s) ? Cette action est irréversible.
                    </p>
