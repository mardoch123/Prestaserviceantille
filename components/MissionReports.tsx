...
const MissionReports: React.FC = () => {
    ...
    const [sortKey, setSortKey] = useState<'date_desc' | 'date_asc' | 'client' | 'service'>('date_desc');
    ...
    const completedMissions = useMemo(() => {
        ...
        if (sortKey === 'date_desc' || sortKey === 'date_asc') {
            return result.sort((a, b) => sortKey === 'date_desc'
        ...
        return result;
    }, [missions, reportMissions, searchQuery, serviceTypeFilter, startDateFilter, endDateFilter, selectedClientFilter, sortKey]);
    ...
    const inProgressMissions = useMemo(() => {
        ...
        return result;
    }, [missions, searchQuery, serviceTypeFilter, startDateFilter, endDateFilter, selectedClientFilter, sortKey]);
    ...
    return (
        <div className="p-8 h-full overflow-y-auto bg-white/40 relative">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6 flex flex-col gap-4">
                ...
                {/* ListingFilterBar — Recherche, tri et filtre client */}
                <ListingFilterBar
                    searchValue={searchQuery}
                    onSearchChange={setSearchQuery}
                    searchPlaceholder="Client, Prestataire, Service..."
                    sortOptions={[
                    ...
                    filterValues={{ client: selectedClientFilter }}
                    onFilterChange={(key, value) => {
                        if (key === 'client') setSelectedClientFilter(value as string);
                    }}
                    filteredCount={displayedMissions.length}
                    totalCount={activeTab === 'completed' ? completedMissions.length + (completedMissions.length - displayedMissions.length) : inProgressMissions.length + (inProgressMissions.length - displayedMissions.length)}
                    ...
                    hasActiveFilters={searchQuery !== '' || selectedClientFilter !== '' || startDateFilter !== '' || endDateFilter !== '' || sortKey !== 'date_desc'}
                />
                                    ...
                                    <div className="bg-white p-4 rounded-xl border border-slate-200">
                                        ...
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <SearchableSelect
                                                ...
                                                onChange={(value) => setDurationMultiplier(Number(value))}
                                                options={[
                                                    { value: '1', label: '×1 (durée normale)' },
                                                    { value: '2', label: '×2' },
                                                    { value: '3', label: '×3' },
                                                    { value: '4', label: '×4' },
                                                ]}
                                                triggerClassName="px-3 py-1 rounded-lg border border-slate-200 text-sm"
                                            />
                                        </div>
