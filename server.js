...
async function sendSMS(phone, message) {
  ...
  const url = `${SMSMODE_BASE_URL}${SMSMODE_ENDPOINT}`;
  console.log('[SMSMode] Calling:', url);
  console.log('[SMSMode] Phone:', formattedPhone, 'Message:', message.substring(0, 50));
...
async function handleSendSMS(req, res) {
  ...
  try {
    ...
    console.log('[api/send-sms] Request:', { phones, msgLen: message?.length });
    ...
    if (message.length > 1000) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'message too long (max 1000 chars)' }));
      return;
    }
